...
const ViewScoresStudent: React.FC<ViewScoresStudentProps> = ({ userID, classID }) => {
    ...
    // Fetch the data when the component mounts or when userID/classID changes
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);  // Ensure the loading state is reset on each fetch
            try {
        ...
        fetchData();
    }, [userID, classID]); // Re-fetch data when userID or classID changes
